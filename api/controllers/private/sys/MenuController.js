...
module.exports = {
  ...
  child: function (req, res) {
    ...
    if (!id)id = '0';
    Sys_menu.find().where({parentId: id}).sort('location asc').sort('path asc').exec(function (err, objs) {
      req.data.unit = objs;
      return res.view('private/sys/menu/child', req.data);
    });
  },
  /**
   * 菜单详情
  ...
  detail: function (req, res) {
    Sys_menu.findOne({id: req.params.id}).exec(function (err, obj) {
      req.data.moment = moment;
      req.data.obj = obj;
      return res.view('private/sys/menu/detail', req.data);
    });
  },
  /**
   * 选择菜单(树形结构)
  ...
  add: function (req, res) {
    var pid = req.query.pid;
    if (pid) {
      Sys_menu.findOne({id: pid}).exec(function (err, obj) {
        req.data.parentUnit = obj;
        return res.view('private/sys/menu/add', req.data);
      });
    } else {
      return res.view('private/sys/menu/add', req.data);
    }
  ...
  addDo: function (req, res) {
    ...
    Sys_menu.findOne({id: parentId}).exec(function (err, unit) {
      ...
      Sys_menu.find().where({parentId: parentId}).sort({path: 'desc'}).limit(1).exec(function (ferr, objs) {
        ...
        Sys_menu.create(body).exec(function (cerr, obj) {
          if (cerr || !obj)return res.json({code: 1, msg: sails.__('add.fail')});
          if (parentId > 0) {
            Sys_menu.update({id: parentId}, {hasChildren: true}).exec(function (e, o) {
            });
          }
  ...
  edit: function (req, res) {
    var id = req.params.id;
    Sys_menu.findOne({id: id}).exec(function (err, obj) {
      if (obj) {
        Sys_menu.findOne({id: obj.parentId}).exec(function (err, punit) {
          req.data.parentUnit = punit;
          req.data.obj = obj;
          return res.view('private/sys/menu/edit', req.data);
        });
      } else {
        return res.view('private/sys/menu/edit', req.data);
      }
  ...
  editDo: function (req, res) {
    var body = req.body;
    Sys_menu.update({id: body.id}, body).exec(function (err, obj) {
    ...
    Sys_menu.findOne(id).exec(function (e, menu) {
      Sys_menu.destroy({path: {like:menu.path+'%'}}).exec(function (err) {
        if (err) {
          ...
          Sys_menu.count({parentId: menu.parentId}).exec(function (err, count) {
            if (count == 0) {
              Sys_menu.update({id: menu.parentId}, {hasChildren: false}).exec(function (err, obj) {
              });
            }
          });
  ...
  enable: function (req, res) {
    var id = req.params.id;
    Sys_menu.update({id: id}, {disabled: false}).exec(function (err, obj) {
      if (err) {
  ...
  sort: function (req, res) {
    Sys_menu.find().sort({location: "asc"}).sort({path: "asc"}).exec(function (err, menus) {
      req.data.menus = menus;
      return res.view('private/sys/menu/sort', req.data);
    });
  },
  sortDo: function (req, res) {
    ...
    ids.forEach(function (obj) {
      ...
      Sys_menu.update({id: id1}, {location: i}).exec(function (e, o11) {
        if (obj.children) {
          obj.children.forEach(function (o2) {
            j++;
            var id2 = o2.id || o2['[id]'];
            Sys_menu.update({id: id2}, {location: j}).exec(function (e, o12) {
              if (o2.children) {
                o2.children.forEach(function (o3) {
                  k++;
                  var id3 = o3.id || o3['[id]'];
                  Sys_menu.update({id: id3}, {location: k}).exec(function (e, o13) {
                  });
                });
              }
            });
